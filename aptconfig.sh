...
Rinstall=false		#R 4.2 installlation
...
if [ $Rinstall = true ]; then
    ...
    gpg --armor --'95C0FAF38DB3CCAD0C080A7BDC78B2DDEABC47B7' | tee /etc/apt/trusted.gpg.d/cran_debian_key.asc # THIS LINE HAD A BUG
...
if [ $initconfig = true ]; then
    ...
    echo "Plug '~/my-prototype-plugin'" >> ~/.config/nvim/init.vim
    echo "call plug#end()" >> ~/.config/nvim/init.vim
    echo ':set number' >> ~/.config/nvim/init.vim
    echo ':set relativenumber' >> ~/.config/nvim/init.vim
    echo ':set clipboard+=unnamedplus "Yanking between windows' >> ~/.config/nvim/init.vim
